...
which typically comes as a tupe (*.pgen, *.pvar.zst, *.psam).
```{r}
library(pgenlibr)
...
# for SNP data, the first two coefficients are 1, the rest are 0
...
For SNP data, the train/validation split is a bit more involved. We must use the
plink software to separate sample.pgen into a training file and a validation file
first. See also https://www.cog-genomics.org/plink/2.0/filter. We recommend 
download the plink excutable https://www.cog-genomics.org/plink/2.0/ for your 
operating system and store it in the same directory as this R Markdown file, 


```{r}
best_fit = fit_with_tuning_real(cbind(covs,snp_matrix), y, standardize=T, intercept=T,
                          nlambda=30, h=0.25, tau=0.5, penalty.factor=penalty.factor,val_ratio=0.2)
best_fit
```
